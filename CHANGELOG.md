...
### Changed
- **The built-in `Explore` agent now uses `openai-codex/gpt-5.4-mini`.** Its standalone prompt is shorter and frames the role around targeted, evidence-backed codebase search while retaining the read-only working-tree invariant. This removes the previous Anthropic-specific default and avoids the rigid prohibition-heavy language that could make the agent refuse useful inspection work.
...
### Removed
- **The built-in `Plan` agent.** Planning remains available through user-defined agents, but it is no longer part of the default roster. User-defined agents named `Plan` continue to work because custom agent names are unrestricted.
